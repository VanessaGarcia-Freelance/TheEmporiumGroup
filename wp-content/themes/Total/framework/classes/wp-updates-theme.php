...
// Get license
$wpex_license = get_option( 'wpex_product_license' ) ? get_option( 'wpex_product_license' ) : get_theme_mod( 'envato_license_key' );

// Return if no license
if ( ! $wpex_license ) {
	return;
}
...
if ( ! class_exists( 'WPUpdatesThemeUpdater_479' ) ) {
	...
	class WPUpdatesThemeUpdater_479 {
		...
		function check_for_update( $transient ) {
			...
			if ( empty( $transient->checked ) || empty( $transient->checked[$this->theme_slug] ) ) {
				return $transient;
			}
...
new WPUpdatesThemeUpdater_479( 'http://wp-updates.com/api/2/theme', basename( get_template_directory() ), $wpex_license );