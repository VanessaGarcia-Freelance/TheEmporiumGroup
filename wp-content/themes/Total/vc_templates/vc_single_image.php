...
if ( ! empty( $lightbox_video ) ) {
	...
	if ( $lightbox_iframe_type ) {
		...
		// Load lightbox js - hum could cause bugs, best not
	...
	// Auto detect style
		...
		// Load lightbox js - hum could cause bugs, best not
		// vcex_inline_js( 'ilightbox_autodetect' );
...
if ( ! empty( $lightbox_custom_img ) ) {
	...
	// Alter link
	$lightbox_custom_img = wp_get_attachment_image_src( $lightbox_custom_img, 'full' );

	if ( isset( $lightbox_custom_img[0] ) ) {
		$link = $lightbox_custom_img[0];
	}
